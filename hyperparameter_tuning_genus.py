...
models = ['rf', 'lgbm', 'xgb', 'svc'] #['lgbm']

# Пример запуска. Для полных геномов
...
# Переменная "full_or_ranged" - признаки фрагментов или геномов используются ('full' для геномов)

training(models, sole_sets, 'uncrossing', 'host', 'full')